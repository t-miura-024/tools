...
//
// herdr 公式プラグイン（../lib/herdr-agent-state.js）は V1 形式のため opencode V2 では
// 直接読み込めない（default exportなし）。公式ファイル自体は herdr 管理なので触らず、
// 走査対象外の lib/ に置いてこのブリッジが V2 の hook / イベントを受けて V1 ハンドラへ
// 転送する（公式ヘッダの指示通り「横に置く」配置の変形）。herdr 再導入で
// plugins/ 直下に復活したら lib/ へ移し直すこと。herdr 側が V2 対応したらこのファイルは削除する。
//
// NOTE: このファイルは default エクスポート（Plugin.define の結果）のみを持つこと。
// opencode のローダーはこの形式を要求し、他の値エクスポートがあると
// ロード全体が失敗する。
import { Plugin } from "@opencode/plugin";
import { HerdrAgentStatePlugin } from "../lib/herdr-agent-state.js";
