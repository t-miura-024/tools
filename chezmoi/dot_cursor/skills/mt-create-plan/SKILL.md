...
Grill Phase の後、作成対象の最終本文と分解要否を提示し、Issue を作成する前にユーザーの明示的な承認を得る。承認がなければ Issue を作成しない。
...
- 親子 Issue の作成前に、分解案と各子の draft 本文についてユーザーの明示的な承認を得る
...
単一計画はユーザー承認後、分解計画は親子まとめてのユーザー承認後に、それぞれを refined へ遷移する:
...
- 子計画の作成や親子を refined へ遷移する前には、必ず明示的な承認を得る
