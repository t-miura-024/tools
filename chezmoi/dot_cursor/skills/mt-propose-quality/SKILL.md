...
```bash
gh label create "kind/plan" --repo <owner/repo> --color "1d76db" --description "計画 Issue" 2>/dev/null || true
```
...
作成した Issue を plans プロジェクトに追加し、Status を `draft` に設定する:

```bash
# Project に追加
gh project item-add <project-number> --owner <owner> --url <issue-url>

# Status を draft に設定（transition-plan.ts のパターンに準拠）
gh api graphql -H "GraphQL-Features: project_v2" \
  ...
  -f fieldId=<statusFieldId> -f optionId=<draftOptionId>
```

`projectId`・`statusFieldId`・`draftOptionId` は `~/.config/mt-plan/config.json` から取得する。
