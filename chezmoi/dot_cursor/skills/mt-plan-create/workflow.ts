  ...
  steps: [
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            purpose: ['計画の全側面についてユーザーと共通認識に達するまでヒアリングを行う（Grill Phase）。'],
            criteria: [],
            approach: [
              '### 1. from-Issue フローの確認',
              '',
            ...
            policy: withDocs
              ? ['repo へのファイル書き込み（CONTEXT.md の更新、ADR ファイルの作成）は禁止。確定した用語・ADR 案はライブ地図の `## 確定用語` / `## ADR 案` セクションに記録する。']
              : undefined,
            input: [`セッションディレクトリ: ${ctx.sessionDir}`, `ライブ地図パス: ${grillMapPath}`],
          });
        },
      },
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            ...
            input: [
              ...
              `ライブ地図: ${join(ctx.sessionDir, GRILL_MAP_KEY)}`,
              `plan-format: ${join(mtPlanDir, 'plan-format.md')}`,
              `domain modeling dir: ${mtDomainModelingDir}`,
            ],
          });
        },
      },
    ...
    // -----------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            purpose: ['起票に必要な環境整備（label 確認）を行い、分解要否の判定材料を artifact に書き出す。'],
            criteria: [],
            approach: [
              '### 1. 対象 repo の確認',
              '',
            ...
            input: [`セッションディレクトリ: ${ctx.sessionDir}`, `対象 repo: ${repoInfo.nameWithOwner}`],
          });
        },
      },
      check: (_ctx: CheckCtx): CheckResult => ({ status: 'pass', reasons: [] }),
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          return buildStepPrompt({
            ...
            policy: ['from-Issue フローでは新規作成せず必ず既存 Issue を更新する。'],
            input: [
              `セッションディレクトリ: ${ctx.sessionDir}`,
              `issue-body: ${join(ctx.sessionDir, ISSUE_BODY_KEY)}`,
              `prepare-decision: ${join(ctx.sessionDir, PREPARE_DECISION_KEY)}`,
            ],
          });
        },
      },
    ...
    // -----------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          return buildStepPrompt({
            purpose: ['計画 Issue を refined に昇格し、作成内容を報告する。'],
            criteria: [],
            approach: [
              '### 1. Issue 番号の確認',
              '',
            ...
            output: ['Issue の URL・番号・対象 repo・Project・label の報告、refined 後の実行案内'],
            input: [`セッションディレクトリ: ${ctx.sessionDir}`, `昇格スクリプト: ${join(mtPlanDir, 'transition-plan.ts')}`],
          });
        },
      },
      check: (_ctx: CheckCtx): CheckResult => ({ status: 'pass', reasons: [] }),
