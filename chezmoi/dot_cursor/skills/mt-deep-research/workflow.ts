  ...
  steps: [
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            purpose: ['事前ヒアリング。調査の背景・目的・前提知識をユーザーから引き出し hearing.md にまとめる。'],
            criteria: [],
            approach: [
              '### 1. ヒアリング本体',
              '',
    ...
    // -----------------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            purpose: ['plan.md を作成し、questions テーブルに 3〜7 個（推奨 5 個）の主要な問いを登録する。'],
            criteria: [],
            approach: [
              '### 担当範囲',
              '',
    ...
    // -----------------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          return buildStepPrompt({
            purpose: ['承認されたすべての問いについて、Researcher SubAgent を並列起動し、調査を実行する。'],
            ...
            output: ['外部通信（外部 URL 取得・SearXNG クエリ）の前に、送信先・データ・目的を宣言する（Researcher SubAgent にも遵守させる）'],
            input: [`セッションディレクトリ: ${ctx.sessionDir}`, `research.db: ${ctx.artifactDbPath ?? '(none)'}`],
          });
        },
      },
      check: (ctx: CheckCtx): CheckResult => {
    ...
    // -----------------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          return buildStepPrompt({
            purpose: ['research サイクル全体の機械監査を実行し、問題があれば Auditor に意味整合性評価を依頼する。'],
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            ...
            criteria: ['auditWriter が pass（report_md_exists / report_md_required_sections / report_md_has_citations / report_md_has_mermaid）'],
            approach: [
              '### 担当範囲',
              '',
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            ...
            criteria: ['auditWriterReviewerCycle が pass（auditWriter + auditReviewer + no_unresolved_must_fix + research_needed_addressed）'],
            approach: [
              '### 手順',
              '',
              '1. 機械監査を実行する',
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            ...
            criteria: ['auditWriterReviewerCycle が pass かつ lint が pass かつ report に禁止コンテンツ（次のアクション/未解決の問い/中間まとめ/SearXNG 信頼性）がないこと'],
            approach: [
              '### 手順',
              '',
