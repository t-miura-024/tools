...
- Backend タスクは合意済み seam での Red → Green を優先する
- テストは public interface の振る舞いを検証し、実装詳細に結合しない
- 水平分割（全テスト先・全実装後）ではなく、1 テスト → 1 実装の縦切りで進める
- typecheck と関連テストをこまめに実行し、レイヤー完了時に広い検証を通す
- 仕様にないことは実装しない
