...
| `grilling` | 人間必須 | 方針・優先度・トレードオフをヒアリングで詰める |
...
1. `mt-grill-rounds` または `mt-grill-with-docs`（`mt-domain-modeling` 併用）で、このマップが辿り着く状態を 1〜2 文に固定する
...
4. 種別に応じて 1 件だけ解く
   - research: 一次資料調査
   - grilling: `mt-grill-rounds` / `mt-grill-with-docs`（`mt-domain-modeling` 併用）
