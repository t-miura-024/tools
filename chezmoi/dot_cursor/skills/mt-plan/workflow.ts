    ...
    // -------------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) =>
          buildStepPrompt({
            purpose: ['計画 Issue の妥当性を検証し、状態を in-progress に遷移して Issue body を読み込む。'],
            criteria: [],
            approach: [
            ...
            output: ['計画番号と Issue body の検証結果・要点の報告'],
    ...
    // -------------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (_ctx: PromptCtx) =>
          buildStepPrompt({
            purpose: ['計画 Issue の `## 📄 ドキュメント` セクションをリポジトリの実ファイルへ転記する。'],
            criteria: [],
            approach: [
              '### 1. ドキュメントセクションの抽出',
              '',
    ...
    // -------------------------------------------------------------------
    {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            purpose: ['専用のレビュアー SubAgent に委譲し、5 観点で客観レビューを行う。'],
            criteria: ['agent-review.json が valid で must 指摘が 0 件であること'],
            approach: [
              '### 1. 証拠収集（スクリプト実行）',
              '',
              '```bash',
            ...
            input: [
              ...
              `collect スクリプト: ${collectScriptPath}`,
              `出力先 JSON: ${jsonPath}`,
              `出力先 MD: ${mdPath}`,
              `レビューラウンド: ${prevRound} → ${nextRound}`,
            ],
          });
        },
      },
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) => {
          ...
          return buildStepPrompt({
            ...
            criteria: ['hunk gate が passes: true であること'],
            approach: [
            ...
            output: [
              `JSON を ${checkPath} に保存し、同じ JSON を report の subagentOutput として返す`,
            ],
            input: [`セッションディレクトリ: ${ctx.sessionDir}`, `出力先: ${checkPath}`],
          });
        },
      },
      check: (ctx: CheckCtx): CheckResult => {
      ...
      task: {
        ...
        buildPrompt: (ctx: PromptCtx) =>
          buildStepPrompt({
            ...
            output: ['Issue の URL・完了作業・未決事項の報告'],
            input: [`セッションディレクトリ: ${ctx.sessionDir}`],
          }),
      },
      check: (_ctx: CheckCtx): CheckResult => ({ status: 'pass', reasons: [] }),
