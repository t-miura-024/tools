...
GitHub Issue ベースの計画作成から実行までを扱う統合スキルです。入力に応じて `mt-plan-create`（計画作成）または `mt-plan-run`（計画実行）にルーティングします。

## ワークフローエンジン

mt-plan-run は `tado` エンジンで手順が管理されています。ワークフロー定義は `mt-plan/workflow.ts` を参照。
...
| 入力 | 委譲先 |
| ---- | ---- |
| 新規計画の目的・背景がある、`draft` の計画 Issue 指定 | `mt-plan-create` |
| `refined` / `in-progress` の計画 Issue 指定 | `mt-plan-run`（エンジン起動） |
| 入力が曖昧 | 本文で選択肢を提示して確認 |

## エンジン起動（mt-plan-run 委譲時）
...
- `draft` の計画は実行せず、先に `mt-plan-create` で整理する
- Sub Issue を持つ親計画は実行せず、子計画を `mt-plan-run` に渡す
