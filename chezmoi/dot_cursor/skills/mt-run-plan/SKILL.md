...
- 作業はすべて `mt-plan-work-executor` SubAgent に委譲する。オーケストレーターはファイル編集を行わず、ミッションの割り振り・進行管理に専念する
- executor SubAgent は可能な範囲で合意済み seam に対する TDD（Red → Green）を使う
- 変更中は typecheck と関連テストをこまめに走らせ、最後に広い検証を 1 回通す
- レビューは全ミッション完了後に Step 4（`review_work`）で自動実行される（SubAgent `mt-plan-work-reviewer` による 5 軸レビュー）。オーケストレーターが別途 `mt-review-diff` を呼ぶ必要はない
- 仕様にない振る舞いは追加しない
...
| 3 | `execute_work` | task | `## 🧩 ミッション` を読み取り、Wave 方式で executor SubAgent（mt-plan-work-executor）を並列起動。再実行時は agent-review.json + human-feedback.json の指摘を統合して修正指示。セクションなしの場合は計画全体を 1 ミッションとして単一委譲 |
...
計画 Issue に `## 🧩 ミッション` セクションがある場合、`execute_work` は各ミッションを Wave 方式に従って executor SubAgent に委譲する。

- 同じ Wave 内のミッションは同一メッセージで複数の Task ツール呼び出しにより並列起動される（最大 5 同時）
- 異なる Wave は番号順に直列実行される（Wave 2 は Wave 1 の全ミッション完了後に開始）
- セクションがない場合は計画全体を 1 ミッションとして単一の executor SubAgent に委譲する
- ミッション間でスコープ（対象ファイル）が重複しないように計画作成時に担保する。フォーマットとルールの詳細は `plan-format.md` の `🧩 ミッション` セクションを参照
