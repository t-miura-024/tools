...
import def, { GATE_SKIP_CONDITIONS, currentGateDecision } from "./index.ts";
...
const LOOP_KEY = "plan_approval_cycle";
const HEAD_KEY = "phase3_planner";
const GATE_KEY = "phase3b_plan_approval";
const JUDGE_KEY = "judge_plan_approval";
const EXHAUSTED_KEY = "plan_approval_exhausted_gate";
...
describe("plan_approval_cycle (revise loop replacement)", () => {
  beforeEach(() => {
  ...
  it("judge は自 loop 外の文脈では error する", () => {
    ...
    const other = stepCheck(JUDGE_KEY)(
      makeCtx({
        ...
        loop: { key: "other_loop", iteration: 1, maxIterations: 3 },
      }),
    );
  ...
  it("Phase 4 の prompt は枯渇マーカーの input を再提示し、枯渇なしでは「なし」とする", () => {
    const buildPrompt = taskStep("phase4_researcher").task.buildPrompt;
