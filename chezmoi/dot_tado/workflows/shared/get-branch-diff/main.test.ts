#!/usr/bin/env bun
/**
 * get-branch-diff/main.ts の自動テスト。
 *
 * 一時ディレクトリ（os.tmpdir + mkdtemp）に Git リポジトリを構築し、
 * `bun run main.ts` を実行して JSON 出力・終了コードを検証する。
...
const SCRIPT = path.join(import.meta.dir, "main.ts");
...
const runScript = (cwd: string, ...args: string[]): ExecResult =>
  exec("bun", ["run", SCRIPT, ...args], cwd);
...
describe("get-branch-diff/main.ts", () => {
  ...
  test("バイナリ変更を処理し、行数はカウントしない", async () => {
    ...
    await commitFile(
      work,
      "bin.dat",
      Buffer.from([0x00, 0x01, 0xff, 0x00, 0x12, 0x34]),
      "modify binary",
    );
  ...
  test("origin/HEAD 未設定ではエラー JSON + 非ゼロ終了", async () => {
    ...
    expect(git(work, "rev-parse", "--verify", "--quiet", "refs/remotes/origin/HEAD").code).not.toBe(
      0,
    );
