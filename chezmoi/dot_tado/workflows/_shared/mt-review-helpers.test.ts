...
describe("isRoundLimitReached (verdict round 上限)", () => {
  test("round > 3 は通過していても true（上限超過は人間判断へ）", () => {
    expect(isRoundLimitReached({ round: 4, passed: true })).toBe(true);
    expect(isRoundLimitReached({ round: REVIEW_ROUND_LIMIT + 1, passed: false })).toBe(true);
  });

  test("round = 3 は未通過のみ true", () => {
    expect(isRoundLimitReached({ round: 3, passed: false })).toBe(true);
    expect(isRoundLimitReached({ round: 3, passed: true })).toBe(false);
  });

  test("round < 3 は常に false", () => {
