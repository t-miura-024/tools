...
it("収集専用ステップを撤去し検証者起動のhookに移す", () => {
  expect(flattenSteps(def.steps).some((step) => step.key === "collect-context")).toBe(false);
  expect(taskStep("run-reviewers").beforeStep).toBeFunction();
});
import { awaitHumanReviewStep, runReviewersStep } from "../review-diff/index.ts";
import { collectPlanReviewContext } from "../shared/collect-plan-review-context/collect-plan-review-context";
import { validateReviewDiff } from "../shared/collect-plan-review-context/validate-review-diff";
...
import { formatReviewComment as formatComment } from "../shared/review-helpers/format-review-comment";
...
import {collectContextStep} from ${JSON.stringify(path.resolve(import.meta.dir, "../review-diff/index.ts"))};
function find(steps) { for(const step of steps) { if(step.key === 'run-reviewers') return step; if(step.type === 'loop') { const found = find(step.body); if(found) return found; } } }
...
`,
  ...
  it("同条件2サイクルで2→1往復、DB登録/上書き、check参照とレビューreportを維持", async () => {
    ...
    for (const baseline of [true, false]) {
      ...
      for (let round = 1; round <= 2; round++) {
        fs.writeFileSync(path.join(repo, "tracked.txt"), `base\nround${round}\n`);
        if (baseline) {
          expect(s.run(["next", ...args]).stepKey).toBe("collect-context");
          ...
          keys.push("collect-context");
        }
        const review = s.run(["next", ...args]);
        expect(review.stepKey).toBe("run-reviewers");
        ...
        expect(s.run(["next", ...args]).stepKey).toBe("run-reviewers");
        if (!baseline)
          expect(
  ...
  for (const failure of ["command", "validation"] as const) {
    it(`${failure}失敗はmaxRetries+1回でfailed/aborted、レビューと後続promptを生成しない`, () => {
      ...
      const db = new Database(path.join(s.home, "workflow.db"), { readonly: true });
      const step = db
        .query("SELECT status FROM steps WHERE session_id=? AND step_key='run-reviewers'")
...
describe("plan-run workflow checks", () => {
  let tmp: string;
  let binDir: string;
...
exit 0`,
  ...
  describe("loop structure (nested human/autonomous cycles)", () => {
    it("外側=人間サイクル / 内側=自律サイクルのネスト loop で、内側先頭が apply-feedback である", () => {
      const outer = loopStep("human-review-cycle");
      expect(outer.phase).toBe("人間サイクル");
      expect(outer.maxIterations).toBe(REVIEW_ROUND_LIMIT);
      expect(outer.onExhausted).toBe("escalate");
      const innerKey = outer.body.find((s) => s.key === "autonomous-review-cycle");
      if (!innerKey || innerKey.type !== "loop") {
        throw new Error("autonomous-review-cycle loop not found in human-review-cycle body");
      }
      ...
      // 内側先頭は新設 apply-feedback（指摘統合＋修正指示組み立て）
      const head = innerKey.body[0];
      expect(head.key).toBe("apply-feedback");
      expect(head.type).toBe("task");
    });

    it("judge は各 loop 本体の末尾に置かれる（分岐点の一元化）", () => {
      const inner = loopStep("autonomous-review-cycle");
      expect(inner.body[inner.body.length - 1].key).toBe("collect-verdict");
      const outer = loopStep("human-review-cycle");
      expect(outer.body[outer.body.length - 1].key).toBe("judge-human");
    });
    ...
    it("loop 外ステップの check は continue を返さない（loop 外 continue の fail-fast）", () => {
      ...
      // 対照: loop 内の継続判定（agent-verdict / collect-verdict）は continue を返す
      for (const key of ["agent-verdict"]) {
      ...
      for (const key of ["judge-human"]) {
        expect(inside.has(key)).toBe(true);
        expect(stepCheck(key).toString()).toContain("judgeGateRework");
      }
    });

    it("identify-plan は loop 外に置かれ、approve/abort のみ持つ（request_changes は巻き戻し不可のため撤去）", () => {
      expect(def.steps.find((s) => s.key === "identify-plan")!.type).toBe("human_gate");
      const question = gateStep("identify-plan").humanGate.questions.find(
        (q) => q.key === "decision",
      )!;
    ...
    it("条件付き loop 内 human_gate の集合を固定する（skip registry の更新強制）", () => {
      ...
      expect(conditionalInnerGates).toEqual(["await-human-review"]);
    });

    it("loop 内 human_gate はいずれかの judge の走査対象にする（収集と作動の統合）", () => {
      // collect（collectGateReworkRequests）は全ゲートキーを動的走査する一方、
      // agent-verdict / judge-human は固定ゲート読みである。新規 loop 内ゲートを
      ...
      const judgeSources = [stepCheck("judge-human").toString()];
  ...
  describe("apply-feedback (inner loop head)", () => {
    const runApplyFeedbackCheck = (artifacts: ArtifactRecord[] = []) =>
      stepCheck("apply-feedback")(makeCtx({ artifacts }));
    ...
    it("統合指示があれば件数を理由に載せて pass する", () => {
      // 人相段階（must=0）のため await-human-review は skip されず、gate 差し戻しと
      // should 指摘の双方を原文被覆する items が必要（双方向の被覆検証）。
      writeFindings({ must: 0, should: 1, want: 0 });
      const filePath = writeFeedback([
        { source: "findings", body: "should detail 0" },
        { source: "gate:await-human-review", body: "修正理由の原文" },
      ]);

      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: decisionAnswers("await-human-review", "request_changes", "修正理由の原文"),
        }),
      );
    ...
    it("buildPrompt は gateAnswers の差し戻し原文と feedback.json 契約を示し、repo 編集と workflow.db を禁じる", () => {
      // 人相段階（must=0）のため await-human-review は skip されず、差し戻しが prompt に載る。
      writeFindings({ must: 0, should: 0, want: 0 });
      const step = taskStep("apply-feedback");
      const prompt = step.task.buildPrompt(
        makePromptCtx({
          gateAnswers: {
            "await-human-review": { decision: { value: "request_changes", input: "直す理由" } },
          },
        }),
      );
    ...
    it("buildPrompt は追加入力の欠落を捏造せず、異常マーカーで記録する", () => {
      ...
      // round-stall-gate の回答が skip 除外されないようにする（世代管理）。
      writeFindings({ must: 1, should: 0, want: 0 });
      writeVerdict({ round: 1 });
      const step = taskStep("apply-feedback");
      const prompt = step.task.buildPrompt(
        makePromptCtx({
          gateAnswers: { "round-stall-gate": { decision: { value: "request_changes" } } },
        }),
      );
    ...
    it("buildPrompt は loop 外ゲートの差し戻しを統合対象外として記録する", () => {
      ...
      const prompt = step.task.buildPrompt(
        makePromptCtx({
          gateAnswers: {
            "identify-plan": { decision: { value: "request_changes", input: "直す" } },
          },
        }),
      );
      expect(prompt).toContain("identify-plan");
    ...
    it("空白のみの追加入力も欠落として fail する", () => {
      ...
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: decisionAnswers("round-stall-gate", "request_changes", "   "),
        }),
      );
      expect(result.status).toBe("fail");
      expect(result.reasons.join("\n")).toContain("追加入力");
    ...
    it("loop 外ゲートの request_changes は fail する（巻き戻し不可の無音消失を防ぐ）", () => {
      const filePath = writeFeedback([{ source: "gate:identify-plan", body: "直す理由" }]);
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: decisionAnswers("identify-plan", "request_changes", "直す理由"),
        }),
      );
      expect(result.status).toBe("fail");
      expect(result.reasons.join("\n")).toContain("loop 外");
    ...
    it("gate 差し戻しがあるのに items が空なら fail する（空素通り防止）", () => {
      // 人相段階（must=0）のため await-human-review は skip されず、差し戻しが収集される。
      writeFindings({ must: 0, should: 0, want: 0 });
      const filePath = writeFeedback([]);
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: decisionAnswers("await-human-review", "request_changes", "直す理由"),
        }),
      );
      expect(result.status).toBe("fail");
    });
    ...
    it("gate 差し戻しの原文が items に含まれなければ fail する（欠落検出）", () => {
      ...
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: decisionAnswers("await-human-review", "request_changes", "直す理由の原文"),
        }),
      );
      expect(result.status).toBe("fail");
      expect(result.reasons.join("\n")).toContain("原文");
    ...
    it("gate 差し戻しの原文を gate:<stepKey> で含めば pass する（一般形の動的走査）", () => {
      ...
      writeFindings({ must: 0, should: 0, want: 0 });
      const filePath = writeFeedback([
        { source: "gate:await-human-review", body: "直す理由の原文" },
      ]);
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
    ...
    it("gate 追加入力＋追記の body は fail する", () => {
      const filePath = writeFeedback([
        { source: "gate:await-human-review", body: "直す理由の原文への追記" },
      ]);
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
    ...
    it("存在しないゲートの source は fail する（gate stepKey の実在確認）", () => {
      const filePath = writeFeedback([{ source: "gate:fake-gate", body: "偽の差し戻し" }]);
      const result = stepCheck("apply-feedback")(
        makeCtx({ artifacts: [artifactRecord("feedback.json", filePath)] }),
      );
      expect(result.status).toBe("fail");
    ...
    it("対応する差し戻しが無い gate item は余剰として fail する", () => {
      ...
      writeFindings({ must: 0, should: 0, want: 0 });
      const filePath = writeFeedback([
        { source: "gate:await-human-review", body: "旧反復の差し戻し残り" },
      ]);
      const result = stepCheck("apply-feedback")(
        makeCtx({ artifacts: [artifactRecord("feedback.json", filePath)] }),
      );
    ...
    it("buildPrompt は outcome 設問以外の差し戻しを載せない（幽霊差し戻しの防止）", () => {
      ...
      const prompt = step.task.buildPrompt(
        makePromptCtx({
          gateAnswers: {
            "await-human-review": { memo: { value: "request_changes", input: "直す理由" } },
          },
        }),
      );
    ...
    it("outcome 設問以外の request_changes は差し戻しにしない（幽霊差し戻しの防止）", () => {
      // def 登録ゲート（await-human-review の outcome は decision）の補助設問だけに
    ...
    it("def 未登録ゲートの request_changes は全キー走査で検出する（見落としの fail-closed）", () => {
      ...
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: {
            "legacy-gate": { memo: { value: "request_changes", input: "直す理由" } },
          },
        }),
      );
      expect(result.status).toBe("fail");
      expect(result.reasons.join("\n")).toContain("legacy-gate");
    });
    ...
    it("型不正の gate 回答は無視して throw しない（無検証 .value アクセスの防止）", () => {
      ...
      const result = stepCheck("apply-feedback")(
        makeCtx({
          artifacts: [artifactRecord("feedback.json", filePath)],
          gateAnswers: {
            "await-human-review": { decision: null },
  ...
  describe("judge-human (outer loop check)", () => {
    it("自律段階では完了へ進めない", () => {
      writeFindings({ must: 1, should: 0, want: 0 });

      const result = stepCheck("judge-human")(makeCtx());
    ...
    it("findings.json が不正なら condition は false（提示せず error へ一本化）で、judge-human の check は error", () => {
      // 不正時に condition が true を返すと、提示→回答→破棄の無駄な往復が固定される。
      // 不正は自律段階の異常であり、gate を提示せず judge-human の check が error で止める
      // （req-2。condition と check の共有関数の分離）。
      fs.writeFileSync(path.join(sessionDir, "findings.json"), "not json");

      expect(gateStep("await-human-review").condition!(makeConditionCtx())).toBe(false);

      const result = stepCheck("judge-human")(
        makeCtx({ gateAnswers: decisionAnswers("await-human-review", "approve") }),
      );
    ...
    it("request_changes なら round を進めず continue する（人間 loop では進めない）", () => {
      ...
      const result = stepCheck("judge-human")(
        makeCtx({ gateAnswers: decisionAnswers("await-human-review", "request_changes", "直す") }),
      );

      expect(result.status).toBe("continue");
      expect(result.reasons.join("\n")).toContain("human-review-cycle");
      expect(result.reasons.join("\n")).toContain("autonomous-review-cycle");
      expect(readEffortRound()).toBe(1);
    });

    it("judge-human は effort.json が無くても continue する（判定と前進の分離）", () => {
      // arch-1: 判定（decideGateRework の純粋写像）と round 前進（advanceReviewRound）は
      // 分離する。人間 loop の継続は前進しないため effort.json に触れず、欠落でも
      // continue する。自律 judge（agent-verdict）は前進するため欠落では error
    ...
    it("未知値は fail する（pass へ丸めない）", () => {
      ...
      const result = stepCheck("judge-human")(
        makeCtx({ gateAnswers: decisionAnswers("await-human-review", "hold") }),
      );
    ...
    it("旧 revise 値は fail し、移行先を案内する（互換受理なし）", () => {
      ...
      const result = stepCheck("judge-human")(
        makeCtx({ gateAnswers: decisionAnswers("await-human-review", "revise") }),
      );
    ...
    it("abort は未知値 fail と分離して error になる（中断意図の記録）", () => {
      ...
      const result = stepCheck("judge-human")(
        makeCtx({ gateAnswers: decisionAnswers("await-human-review", "abort") }),
      );
    ...
    it("型不正の回答値は TypeError にせず error になる（無検証 .value アクセスの防止）", () => {
      ...
      const malformed: GateAnswers[] = [
        { "await-human-review": { decision: null } } as unknown as GateAnswers,
        { "await-human-review": { decision: { value: 123 } } } as unknown as GateAnswers,
      ];
      for (const gateAnswers of malformed) {
        const result = stepCheck("judge-human")(makeCtx({ gateAnswers }));
        expect(result.status).toBe("error");
        expect(result.reasons.join("\n")).toContain("回答がありません");
      }
    });
  ...
  describe("start-difit-review", () => {
    ...
    it("buildPrompt は URL 提示と再入時のサーバ再利用を指示する", () => {
      const step = taskStep("start-difit-review");
  ...
  describe("run-reviewers.beforeStep の収集契約", () => {
    ...
    it("レビュー判断・並列起動・reportの契約は同じStepを継承", () => {
      const step = taskStep("run-reviewers");
  ...
  describe("resolve-effort (human_gate 廃止 — Issue body コメント or medium/medium)", () => {
    it("human_gate を持たず task 型である", () => {
      const step = taskStep("resolve-effort");
      expect(step.type).toBe("task");
    });
    ...
    it("HTML コメントがあれば derived で pass", () => {
      ...
      const result = stepCheck("resolve-effort")(makeCtx());
      expect(result.status).toBe("pass");
  ...
  describe("execute-work (loop 反復の round 前進なし)", () => {
    ...
    it("execution-result.json の申告が無ければ fail する", () => {
      const result = stepCheck("execute-work")(makeCtx());
    ...
    it("gate 差し戻しがあるのに feedback.json がなければ fail する（apply-feedback との接続）", () => {
      // 人相段階（must=0）のため await-human-review は skip されず、差し戻しが収集される。
      writeFindings({ must: 0, should: 0, want: 0 });
      const resultPath = path.join(sessionDir, "execution-result.json");
      fs.writeFileSync(
      ...
      const result = stepCheck("execute-work")(
        makeCtx({
    ...
    it("gate 差し戻しがあり feedback.json の items が非空なら pass する", () => {
      ...
      fs.writeFileSync(
        path.join(sessionDir, "feedback.json"),
        JSON.stringify({ items: [{ source: "gate:await-human-review", body: "直す理由" }] }),
      );
      const result = stepCheck("execute-work")(
        makeCtx({
          artifacts: [artifactRecord("execution-result.json", resultPath)],
    ...
    it("buildPrompt は feedback.json を修正ソースの先頭に置き、workflow.db/sqlite を指示しない", () => {
      const step = taskStep("execute-work");
      const prompt = step.task.buildPrompt(makePromptCtx());
      expect(prompt).toContain("feedback.json");
      expect(prompt).toContain("apply-feedback");
    ...
    it("gate 差し戻しの原文が items に無ければ非空でも fail する（被覆の再検証）", () => {
      ...
      fs.writeFileSync(
        path.join(sessionDir, "feedback.json"),
        JSON.stringify({
          items: [{ source: "gate:await-human-review", body: "無関係な修正指示" }],
        }),
      );
      const result = stepCheck("execute-work")(
        makeCtx({
    ...
    it("findings should-only は自律対象外のため feedback.json が空でも pass する", () => {
      // apply-feedback は must のみ必須・should/want は任意。execute-work の needsFeedback も
    ...
    it("findings must があるのに無関係 body のみなら非空でも fail する（dummy すり替えの検出）", () => {
      ...
      // する。execute-work 側でも apply と同じ期待で厳密被覆を再検証する。
      writeFindings({ must: 1, should: 0, want: 0 });
      const resultPath = path.join(sessionDir, "execution-result.json");
      fs.writeFileSync(
        resultPath,
  ...
  describe("normalize-findings (Step import)", () => {
  ...
  describe("await-human-review (Step import: plan-run が condition を override)", () => {
    const conditionOf = () => gateStep("await-human-review").condition!;
    ...
    it("condition: findings.json を読めない場合は false（提示せず judge の error へ一本化）", () => {
      ...
      // judge-human の check が findings を再検証して error で止める（req-2）。
      expect(conditionOf()(makeConditionCtx())).toBe(false);
    });

    it("condition は plan-run が override する（review-diff の step は condition を持たず単独では常に提示）", () => {
      const step = gateStep("await-human-review");
    ...
    it("reviseTargetStep を持たず、request_changes が judge-human 経由の差し戻しを案内する", () => {
      const step = gateStep("await-human-review");
      // review-diff 単独では reviseTargetStep を持たない（plan-run も付与しない）
      ...
      // 入力した修正理由の消費先（gateAnswers → apply-feedback の feedback.json）を案内する
      expect(requestChanges.desc).toContain("judge-human");
    ...
    it("人間サイクル（外側 loop 本体）に置かれる", () => {
      const outer = loopStep("human-review-cycle");
      expect(outer.body.some((s) => s.key === "await-human-review")).toBe(true);
      const inner = loopStep("autonomous-review-cycle");
      expect(inner.body.some((s) => s.key === "await-human-review")).toBe(false);
    });
  });

  describe("自律上限から人間レビューへの引き渡し", () => {
    const humanAnswers = (value: string) =>
      decisionAnswers("await-human-review", value, "修正してください");
    ...
    async function normalizeClock(iteration: number) {
      await taskStep("normalize-findings").beforeStep!({
        ...makePromptCtx(),
        stepKey: "normalize-findings",
        attemptNumber: 1,
        loop: { key: "autonomous-review-cycle", iteration, maxIterations: REVIEW_ROUND_LIMIT },
      });
    }
    ...
    it("自律5ラウンドを消費すると既存登録・verdict検証後に人間ゲートへ進む", async () => {
      ...
      for (let round = 1; round <= REVIEW_ROUND_LIMIT; round++) {
        ...
        expect(stepCheck("agent-verdict")(makeCtx()).status).toBe(
      ...
      expect(stepCheck("collect-verdict")(makeCtx()).status).toBe("pass");
      expect(doneCalled()).toBe(false);
      expect(gateStep("await-human-review").condition!(makeConditionCtx())).toBe(true);
      const keys = loopStep("autonomous-review-cycle").body.map((step) => step.key);
      expect(keys.slice(keys.indexOf("agent-verdict"))).toEqual([
        "agent-verdict",
        "start-difit-review",
        "collect-verdict",
      ]);
    ...
    it("must=0・should/want残存でも自律反復せず人間へ渡しセッションを保持する", () => {
      ...
      expect(stepCheck("agent-verdict")(makeCtx()).status).toBe("pass");
      expect(stepCheck("collect-verdict")(makeCtx()).status).toBe("pass");
      expect(doneCalled()).toBe(false);
      expect(gateStep("await-human-review").condition!(makeConditionCtx())).toBe(true);
    });
    ...
    it("人間差し戻し1〜4回後の自律予算は1から5で、round更新は冪等", async () => {
      ...
      for (let human = 1; human < REVIEW_ROUND_LIMIT; human++) {
        ...
        expect(
          stepCheck("judge-human")(makeCtx({ gateAnswers: humanAnswers("request_changes") }))
            .status,
        ).toBe("continue");
        expect(doneCalled()).toBe(false);
        for (let round = 1; round <= REVIEW_ROUND_LIMIT; round++) {
    ...
    it("実tado next/reportで自律5回×人間5回を進め、最後の差し戻しだけpausedになる", () => {
      ...
      fs.writeFileSync(
        ...
        `
import def from ${JSON.stringify(path.join(import.meta.dir, "index.ts"))};
const human = def.steps.find(s => s.key === "human-review-cycle");
const autonomous = human.body.find(s => s.key === "autonomous-review-cycle");
const normalize = autonomous.body.find(s => s.key === "normalize-findings");
const verdict = autonomous.body.find(s => s.key === "agent-verdict");
const judge = human.body.find(s => s.key === "judge-human");
...
export default {
  ...
  steps: [{ ...human, body: [
    ...
    { ...judge, task, check: ctx => judge.check({ ...ctx, gateAnswers: {
      "await-human-review": { decision: { value: "request_changes", input: "test fixture" } },
    } }) },
  ] }],
};
`,
      ...
      for (let human = 1; human <= REVIEW_ROUND_LIMIT; human++) {
        for (let round = 1; round <= REVIEW_ROUND_LIMIT; round++) {
          ...
          expect(next.stepKey).toBe("normalize-findings");
          ...
          expect(report("normalize-findings").nextAction).toBe("continue");
          expect(run(["next", ...sessionArgs]).stepKey).toBe("agent-verdict");
          expect(report("agent-verdict").nextAction).toBe(
            round < REVIEW_ROUND_LIMIT ? "repeat" : "continue",
          );
        }
        const next = run(["next", ...sessionArgs]);
        expect(next.stepKey).toBe("judge-human");
        expect(next.context.loop.iteration).toBe(human);
        const result = report("judge-human");
    ...
    it("findingsにmustが残っていても人間が最新difitで解決済みなら承認・cleanupできる", () => {
      ...
      expect(
        stepCheck("judge-human")(makeCtx({ gateAnswers: humanAnswers("approve") })).status,
      ).toBe("pass");
    ...
    it("doneが非通過かつblocking空（判定不能出力）なら検証済みでも承認しない", () => {
      ...
      const result = stepCheck("judge-human")(makeCtx({ gateAnswers: humanAnswers("approve") }));
      expect(result.status).toBe("fail");
    ...
    it("上限時も破損verdict・不一致・task失敗を人間レビューへの正常通過に変換しない", () => {
      ...
      fakeMtDifitGate({ checkJson: '{"passes":true,"blocking_threads":[]}' });
      expect(stepCheck("collect-verdict")(makeCtx()).status).toBe("fail");
      expect(
        stepCheck("collect-verdict")(makeCtx({ attemptResult: { status: "failed" } })).status,
      ).toBe("error");
      fs.writeFileSync(path.join(sessionDir, "verdict.json"), "invalid");
      expect(stepCheck("collect-verdict")(makeCtx()).status).toBe("error");
      expect(doneCalled()).toBe(false);
    });

    it("finalize-doneは旧再計画artifactを要求せず承認後のIssue閉鎖を検証する", () => {
      writeScript("gh", `printf '%s\\n' '{"state":"CLOSED"}'`);
      const file = path.join(sessionDir, "plan-number.txt");
      fs.writeFileSync(file, "97");
      const result = stepCheck("finalize-done")(
        makeCtx({ artifacts: [artifactRecord("plan-number.txt", file)] }),
      );
      expect(result.status).toBe("pass");
      expect(taskStep("finalize-done").task.buildPrompt!(makePromptCtx())).not.toContain(
        "replan-plan-number",
      );
    });
  });
...
describe("execute-work (difit feedback)", () => {
  it("再実行時の修正ソースに feedback.json（apply-feedback の統合指示）を先頭に明記し、workflow.db を指示しない", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mt-plan-feedback-"));
    try {
      const step = taskStep("apply-feedback");
      expect(step).toBeDefined();
      const execStep = taskStep("execute-work");
