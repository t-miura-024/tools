...
import * as mod from "./04.01.04-run-reviewers.ts";
import {
  makeSnapshotCtx,
...
describe("04.01.04-run-reviewers prompt snapshot", () => {
  ...
  it("snapshot", () => {
    const effortPath = path.join(sessionDir, "effort.json");
    fs.writeFileSync(effortPath, JSON.stringify({ width: "medium", depth: "medium", round: 1 }));
