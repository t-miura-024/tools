...
function createCollectVerdictStep(humanReviewPending: boolean): TaskStepDef {
  return {
    key: "collect_verdict",
    phase: "verdict 収集",
    type: "task",
    ...
    task: {
      ...
      buildPrompt: (ctx: PromptCtx) => {
        ...
        return [
          ...
          "```",
          humanReviewPending
            ? `4. 自律上限 ${REVIEW_ROUND_LIMIT} 到達時は残 must / should / want を difit 上に残して await_human_review へ渡す。追加ゲートは設けない。`
      ...
      if (findingsResult2.valid) {
        ...
        if (verdict.round !== findingsResult2.parsed!.round) {
          return {
            status: "fail",
            reasons: [
              `verdict.json の round=${verdict.round} が findings.json の round=${findingsResult2.parsed!.round} と不一致です。round は findings.json（effort.json を継承。mt-plan-run では自律 loop の反復番号）から引き継いでください`,
            ],
          };
        }
  ...
  steps: [
    ...
    // -------------------------------------------------------------------
    {
      ...
      body: [
        ...
        createCollectVerdictStep(false),
...
export const completeHumanReviewStep: TaskStepDef = {
  ...
  check: (ctx: CheckCtx): CheckResult => {
    ...
    // ゲート結果を取得できていない出力（判定不能。単独 collect_verdict の done 消費と同じ
