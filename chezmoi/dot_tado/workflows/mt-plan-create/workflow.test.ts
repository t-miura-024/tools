...
describe("mt-plan-create workflow structure", () => {
  ...
  it("review_cycle → review_exhausted → create_refined → finalize の順序で動作する", () => {
    const keys = def.steps.map((s) => s.key);
    expect(keys).toEqual(["review_cycle", "review_exhausted", "create_refined", "finalize"]);
  });

  it("review_cycle の body は grill 先頭・judge 末尾で作業ステップ群＋ゲートを含む", () => {
    const bodyKeys = loopStep("review_cycle").body.map((s) => s.key);
  ...
  it("review_gate は Issue 実物ではなく session ファイルを対象にする", () => {
    const gate = gateStep("review_gate");
  ...
  it("review_body の buildPrompt は 6 観点と must/should/want 重み付けを指示する", () => {
    ...
    const step = taskStep("review_body");
  ...
  it("review_body の buildPrompt は自己レビューの限界と must 残存時の request_changes 経路を明示する", () => {
    const repoInfoPath = path.join(sessionDir, "repo-info.json");
    fs.writeFileSync(
      repoInfoPath,
      JSON.stringify({ owner: "someone", repo: "x", nameWithOwner: "someone/x" }),
  ...
  it("create_refined の buildPrompt は must 残存到達時の escalate と自動集約の否定を含む", () => {
    const step = taskStep("create_refined");
  ...
  it("create_refined の buildPrompt は must 残存時の escalate→request_changes 誘導に一本化する", () => {
    const step = taskStep("create_refined");
    const prompt = step.task.buildPrompt(makePromptCtx());
    expect(prompt).toContain("escalate");
    expect(prompt).toContain("review_gate の request_changes");
  ...
  it("create_refined の check は枯渇経由の must 残存を警告として記録し must 理由では fail しない", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ loop: "review_cycle", gate: "review_gate", iteration: 3 }),
    );
    const result = stepCheck("create_refined")(
  ...
  it("create_refined の check は枯渇マーカー破損で error に倒す（fail-open しない）", () => {
...
describe("mt-plan-create review_cycle loop", () => {
  let tmp: string;
  let sessionDir: string;
  ...
  function judgeCtx(value: string | undefined, input?: string, iteration = 1): CheckCtx {
    const gateAnswers = value === undefined ? {} : answers("review_gate", value, input);
    return makeCtx({
      gateAnswers,
      loop: { key: "review_cycle", iteration, maxIterations: 3 },
    });
  }

  const judgeCheck = () => {
    const step = flattenSteps(def.steps).find((s) => s.key === "judge_review");
    if (!step || step.type === "loop") throw new Error("judge_review not found");
    return step.check;
  };

  it("loop は maxIterations=3・onExhausted=escalate である", () => {
    const loop = loopStep("review_cycle");
  ...
  it("judge は request_changes→continue する（上限前）", () => {
    for (const iteration of [1, 2]) {
      ...
      expect(result.reasons.join("\n")).toContain("rewind review_cycle to grill");
    }
  });
  ...
  it("judge は最終反復の request_changes で枯渇マーカーを残して pass する", () => {
    ...
    expect(marker.loop).toBe("review_cycle");
    expect(marker.gate).toBe("review_gate");
  ...
  it("judge は他ゲートの request_changes を拾わない（幽霊差し戻し防止）", () => {
    const result = judgeCheck()(
      makeCtx({
        gateAnswers: {
          ...answers("review_gate", "approve"),
          ...answers("review_exhausted", "request_changes", "別ゲートの入力"),
        },
        loop: { key: "review_cycle", iteration: 1, maxIterations: 3 },
      }),
    );
    expect(result.status).toBe("pass");
  });
  ...
  it("judge は自 loop 外の文脈では error する", () => {
    const outside = judgeCheck()(
      makeCtx({ gateAnswers: answers("review_gate", "approve"), loop: null }),
    );
    expect(outside.status).toBe("error");
    expect(outside.reasons.join("\n")).toContain("review_cycle");
  });

  it("review_exhausted の condition は枯渇マーカーがある反復でのみ true になる", () => {
    const step = gateStep("review_exhausted");
    if (!step.condition) throw new Error("review_exhausted has no condition");
  ...
  it("review_exhausted の condition は別 loop・別 gate・マーカー破損では throw する（fail-closed）", () => {
    const step = gateStep("review_exhausted");
    const condition = step.condition;
    if (!condition) throw new Error("review_exhausted has no condition");
    fs.writeFileSync(
      path.join(sessionDir, "review-cycle-exhausted.json"),
      JSON.stringify({ loop: "other_loop", gate: "review_gate", iteration: 3 }),
    );
    expect(() => condition(makeCtx())).toThrow();
    fs.writeFileSync(
      path.join(sessionDir, "review-cycle-exhausted.json"),
      JSON.stringify({ loop: "review_cycle", gate: "other_gate", iteration: 3 }),
    );
    expect(() => condition(makeCtx())).toThrow();
    fs.writeFileSync(
      path.join(sessionDir, "review-cycle-exhausted.json"),
      JSON.stringify({ loop: "review_cycle", gate: "review_gate", iteration: 2 }),
    );
    expect(() => condition(makeCtx())).toThrow();
    fs.writeFileSync(path.join(sessionDir, "review-cycle-exhausted.json"), "not-json");
    expect(() => condition(makeCtx())).toThrow();
  ...
  it("review_exhausted の condition は iteration overshoot（>max）でも true になる（fail-closed）", () => {
    const step = gateStep("review_exhausted");
    const condition = step.condition;
    if (!condition) throw new Error("review_exhausted has no condition");
    fs.writeFileSync(
      path.join(sessionDir, "review-cycle-exhausted.json"),
      JSON.stringify({ loop: "review_cycle", gate: "review_gate", iteration: 4 }),
    );
  ...
  it("review_exhausted は approve/abort のみを持ち request_changes を持たない", () => {
    const gate = gateStep("review_exhausted");
    const questions = gate.humanGate.questions;
    expect(questions.map((q) => q.key)).toEqual(["decision"]);
    expect(gate.humanGate.outcomeQuestionKey).toBe("decision");
  ...
  it("review_gate は request_changes（input 必須）を持ち続ける", () => {
    const gate = gateStep("review_gate");
  ...
  it("grill の prompt は request_changes の追加入力を注入し、初回は「なし」とする", () => {
    ...
    const withFeedback = step.task.buildPrompt(
      makePromptCtx({
        ...
        gateAnswers: answers("review_gate", "request_changes", "スコープを見直すこと"),
      }),
    );
    expect(withFeedback).toContain("スコープを見直すこと");
    expect(withFeedback).toContain("review_gate");
  ...
  it("loop 外ステップの check は continue を返さない", () => {
    for (const key of ["review_exhausted", "create_refined", "finalize"]) {
      const step = flattenSteps(def.steps).find((s) => s.key === key);
