...
// -----------------------------------------------------------------
// Step 1c: 差し戻し判定（analysis-cycle の loop 末尾）
//   confirm-analysis の gateAnswers を読んで分岐する loop の check。
//   request_changes → 判定 `continue` で loop 先頭（grill）へ巻き戻る。
//   上限到達時は枯渇マーカーを残して pass で脱出し、loop 外の
//   analysis-exhausted へ渡す。4分岐に pass フォールバックは設けない。
// -----------------------------------------------------------------
export const judgeAnalysisStep: TaskStepDef = {
  key: "judge-analysis",
  phase: "差し戻し判定",
  type: "task",
  maxRetries: 0,
  ...
  check: (ctx: CheckCtx): CheckResult =>
    judgeGateRework(ctx, {
      gateKey: "confirm-analysis",
      loopKey: "analysis-cycle",
      headKey: "grill",
      markerKey: "analysis-cycle-exhausted.json",
      exhaustedKey: "analysis-exhausted",
    }),
};
