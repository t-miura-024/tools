...
const LOOP_KEY = "candidate_cycle";
const HEAD_KEY = "brainstorm";
const GATE_KEY = "present_gate";
const JUDGE_KEY = "judge_present";
const EXHAUSTED_KEY = "present_exhausted_gate";
...
describe("candidate_cycle (revise loop replacement)", () => {
  ...
  it("confirm_done は対象外のため approve/abort のまま変更しない", () => {
    const question = gateStep("confirm_done").humanGate.questions.find(
      (q) => q.key === "decision",
    )!;
    const values = (question.choices ?? []).map((c) => c.value).sort();
    expect(values).toEqual(["abort", "approve"]);
