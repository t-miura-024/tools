...
//! state 未追跡（hunk-review.json なしでセッションは live）では active と注記を表示し、
...
fn render_status(repo_root: &Path) -> String {
    let live = shared::find_session(repo_root).is_ok();

    let Some(state) = shared::read_review_state(repo_root) else {
        if live {
            // state 未追跡: `mt hunk start` 前でも live なら active と注記を返す
            return "hunk review session: active\n  note: hunk-review.json なし（applied 数は mt hunk start 後に追跡）\n"
                .to_string();
        }
