...
# review-diff の指摘を差分内の `+` 行のみに限定する

## 背景 (Context)

`review-diff` は差分を敵対的に検証するワークフローであるが、検証者（`mt-review-diff-reviewer` SubAgent）が `edit:deny / bash:deny` ながら `read` は自由であったため、差分に含まれないファイル・行への指摘が混入し、difit コメントのノイズとなっていた。`collect_context` が生成する `diff.txt`（target なしは `base...HEAD + unstaged + untracked`、target ありは `base...target`。範囲の契約は ADR-0026「検証対象 diff.txt の提示範囲と完全性（target あり / なし）」参照）が SoT であるにもかかわらず、プロンプトにも機械的検証にも「差分外禁止」の規律がなく、位置情報（`filePath` / `position`）も任意（`general` 許容、`side:old` 許容）であった。

観点プール 15 観点のうち `logic-3 影響範囲` は定義上「同種問題を抱える他箇所（横展開）」を誘発し、`arch-1〜5` も差分外の設計論に触れがちで、差分外指摘を助長していた。差分外指摘は difit 上での可視性・解決運用を複雑にし、人間レビューの負荷を上げるため、明確に禁止する必要が生じた。

## 決定 (Decision)

`review-diff` の指摘は `diff.txt` の `+` 行（追加/変更行）のみに限定し、機械的フィルタとプロンプト強化の二重防御で強制することとした。

- **SoT**: `diff.txt` を唯一の SoT とする。`get-branch-diff.ts` 等との二重管理は行わない。
- **粒度**: 行単位（厳格） — `+` 行のみ許容。差分コンテキスト行（` ` 3行）は含めない。ファイル単位の許容は行わない。
- **読み取り vs 指摘**: 読み取りは自由、出力（`findings` / difit コメント）のみを差分内に制限する。文脈理解のための read は許容する。
- **強制**: プロンプト強化 + `normalize_findings` での機械的フィルタの複合。`shared/review-helpers.ts` に純粋関数 `parseDiffChangedLines(diffRaw): Map<file, Set<addedLines>>` と `filterFindingsByDiff(findings, map): {kept, filteredOut}` を追加し、`diff --git` / `+++ b/<path>` / `@@` の new側カウントで `+` 行を抽出する。削除ファイル（`+++ /dev/null`）、バイナリ（`Binary files`）、リネームは新パス（`+++ b/<new>`）のみを対象とする。`+++` 行のパスは `core.quotePath=false` の生パスを基本とし、`"` 等を含み C-quote された `"b/<path>"` 形は `parseDiffHeaderPath` が逆写像して、検証者が返す生の filePath とキーを一致させる。
- **位置情報**: `filePath` 必須、`position: {side:"new", line}` 必須（line は `+` 行の行番号）。`general`（ファイルなし）/ `side:"old"` / `position` なしは禁止。後方互換のフォールバック（`general` → `position` 合成）は AGENTS.md 方針に従い削除する。
...
- **実装範囲**: `workflows/review-diff/index.ts`（`run_reviewers` / `normalize_findings` プロンプト）、`shared/review-helpers.ts`（純粋関数・validate・difit コメント変換）、`agents/mt-review-diff-reviewer.md`（3箇所同期）、本 ADR。
...
- `findings.json` の全指摘は `filePath` と `side:"new"` の `+` 行に紐付き、difit コメントは全て新コード上で可視化・解決可能になる。`general` / `old` への指摘は機械的に除外され、差分外ノイズが消失する。
