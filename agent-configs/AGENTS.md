...
ユーザーレベルの設定デプロイ先を直接編集してはならない。以下は `tools/agent-configs/` から同期されるため、直接編集すると次回同期時に上書きされる。

| Source of Truth | **直接編集禁止** |
|---|---|
| `tools/agent-configs/` | `~/.cursor/` `~/.config/opencode/` `~/.claude/` |

設定変更時は必ず `tools/agent-configs/` を編集し、`mt agent-config sync` で同期する。
